...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  if (user.status !== 'active') {
    return next(new ErrorResponse(`Account status is '${user.status}'. Access denied.`, 403));
  }
...
exports.register = asyncHandler(async (req, res, next) => {
  ...
  if (!username || !firstName || !lastName || !email || !password || !phoneNumber || !address || !role || !sex) {
      return next(new ErrorResponse('Missing required fields: username, firstName, lastName, email, password, phoneNumber, address, role, sex', 400));
  }
